const projects = [
  // ── FINANCE & FP&A ──────────────────────────────────
  // Two flagships, in the order of the FP&A cycle: plan first, then close.
  {
    id: 'fpa-planning-model',
    category: 'finance',
    featured: true,
    title: 'Driver-Based Planning: Headcount, Cloud and Runway for a SaaS Scale-Up',
    shortDesc: 'An 18-month driver-based plan for a SaaS scale-up: how much to hire, where, and what to commit on cloud, tested under three scenarios against cash runway and gross margin guardrails.',
    fullDesc: 'Northlight is a fictional USD 24M ARR software company with teams in Montreal, Paris and Austin. The plan is built from drivers rather than typed growth rates: every seat is costed by role and location (salary, employer taxes from published 2026 rate tables, benefits, ramp, merit), new customers come from ramped sales capacity, cloud cost comes from compute units per customer, and cash follows the P&L through receivables and deferred revenue. Three hiring plans crossed with two cloud purchasing choices run under base, upside and downside scenarios. Two guardrails decide, 18 months of downside runway and a 74% gross margin floor, and the model recommends the phased hiring plan with a one-year cloud commitment. The same build produces a monthly actual-versus-budget bridge that ties to the cent, a versioned assumption log, an Excel workbook with live formulas, a one-page CFO memo and a five-slide board pack. No language model anywhere: the finance machinery is the point.',
    highlights: [
      'Recommendation generated by the model: phased hiring plus a cloud commitment; the front-loaded plan fails the downside runway guardrail by about a month',
      ...
      'Actual-versus-budget bridge splits every line into volume, price, headcount, rate and usage; the components sum to the EBITDA variance to the cent',
      'Each assumption carries a source label (published benchmark, derived rate, or judgment call) and a change log that replays onto the budget snapshot',
      'Excel export with live formulas, recalculated by Excel in the test suite; every number on the case study traces to a model output'
    ],
    skills: ['Driver-Based Planning', 'Scenario Modeling', 'Workforce Planning', 'SaaS Metrics', 'Cash Runway', 'Python', 'Excel'],
    thumb: 'images/projects/fpa-planning-scenarios.webp',
    hover: 'images/projects/fpa-planning-guardrails.webp',
